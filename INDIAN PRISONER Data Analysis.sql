...
sum(case when type='Under Trials' then Total else 0 end)'Under Trials Prisioner'
 ...
 -- / Q3. Find early Female and Male Prisoner and Their Percentage.
select *,
...
sum(case when gender='Male' then Total else 0 end) as 'Male_Prisioner'
 ...
 -- / Q5.  Total No of Inmates try escaping from prison and their percentage with total Number  
  ...
  select *,
  concat(round((No_of_inmates_esacape/(Total_prisoner+No_of_inmates_esacape))*100,2)," %") as "Percentage_of_Inmate_Escape" 
   from
  ...
  -- / Q6.  States from which inmates mostly try escaping (categories in Male and Female) and gives ranking order by total.
   select *,
